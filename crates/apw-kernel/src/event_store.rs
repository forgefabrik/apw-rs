...
impl EventStore for MemoryEventStore {
    ...
    fn replay(&self, from: Tick, to: Tick) -> Result<Vec<EventEnvelope>> {
        Ok(self.events.iter().filter(|e| e.tick >= from && e.tick < to).cloned().collect())
    }
    fn snapshot(&self, _at: Tick) -> Result<Vec<u8>> { Ok(Vec::new()) }
    fn restore(&mut self, _at: Tick, _data: &[u8]) -> Result<()> { Ok(()) }
}
